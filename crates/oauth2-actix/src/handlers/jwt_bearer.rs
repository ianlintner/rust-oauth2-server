...
use oauth2_core::{
    Actor as DelegationActor, OAuth2Error, ProtectedResource, TokenResponse, TrustedIssuer, User,
};
...
pub(crate) async fn handle_jwt_bearer_grant(
    ...
    agent_config: Option<web::Data<AgentConfig>>,
    ...
    // Task 9 registers `AgentConfig` as app_data; until then fall back to the
    // environment-derived defaults so this grant works standalone.
    let default_agent_config;
    let agent = match agent_config.as_ref() {
        Some(cfg) => cfg.as_ref(),
        None => {
    ...
    let resource = match (req.resource.as_deref(), resource_ceiling.is_empty()) {
        (Some(requested), false) => {
            if !resource_ceiling.iter().any(|r| r == requested) {
                return Err(OAuth2Error::new(
                    "invalid_target",
                    Some("requested resource is not covered by the assertion"),
                ));
            }
            Some(requested.to_string())
        }
        (Some(requested), true) => Some(requested.to_string()),
        (None, false) => resource_ceiling.first().cloned(),
        (None, true) => None,
    };
    if let Some(ref uri) = resource {
        validate_requested_resource(storage.as_ref(), uri).await?;
    }
    ...
    if let Some(uri) = resource {
        body["resource"] = Value::String(uri);
    }
...
/// RFC 8707 §2 shape check plus, when the registry is populated, a
/// registered-resource check.
///
/// Task 9 introduces `token_exchange::resolve_requested_resources`, which
/// subsumes this; de-duplicate at integration.
async fn validate_requested_resource(
    storage: &DynStorage,
    resource: &str,
) -> Result<(), OAuth2Error> {
    ProtectedResource::validate_uri(resource)?;
    let registry = storage.list_resources().await?;
    if !registry.is_empty() && !registry.iter().any(|r| r.resource_uri == resource) {
        return Err(OAuth2Error::new(
            "invalid_target",
            Some("requested resource is not registered"),
        ));
    }
    Ok(())
}
