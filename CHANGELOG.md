...
- RFC 9700 Phase 6.11 — redact PII from scope field in token introspection responses (#199)
...
- RFC 9700 Phase 6.13 — RFC 8252 native-app redirect URI tightening (#204)
- observability: paved-path baseline Prometheus metrics + Kubernetes scrape annotations (#205)
...
- OpenTelemetry (OTEL) Wave 1 — OTLP export, W3C tracecontext propagation, log↔trace linking (#228)
- GitOps + Flux image automation for bigboy AKS cluster (#230)
- fix: app_info Prometheus gauge reads version from Cargo workspace root (#233)
...
- k8s: add CSI SecretProviderClass (Azure Key Vault) and wire Google/GitHub OAuth2 env vars (#262)
- security: mTLS client certificate Subject DN validation (RFC 8705 §2.1) + DPoP-bound token introspection (RFC 9449 §7.1) (#263)
